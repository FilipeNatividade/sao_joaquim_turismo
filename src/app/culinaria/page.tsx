...
import { QUERY, QUERY_LIST_COOKING } from './query'
...
const Page = () => {
  ...
  const getDataList = async () => {
    const responseList = await Fetcher(QUERY_LIST_COOKING);
    const dataList = responseList?.queryRestauranteContents;
  ...
  return (
    <main className={S.container} >
      ...
      {dataList && dataList.length > 0 ? (<ListContent content={dataList} origin='culinaria' categoria={false} />
      ) : (
        dataList !== null && dataList !== undefined && dataList.length === 0 && <p className='message_error'>Restaurante não encontrada para o publico desejado</p>
      )}
      <Enphasis />
      {data?.titulo?.iv && <Navigation local='Nossa gastronomia' />}
    </main>
  )
}
